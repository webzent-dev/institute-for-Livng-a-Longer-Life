...
class ContentManagementController extends Controller
{
    ...
    public function index()
    {
        ...
        $termsandconditionsPageContent = ContentManagement::where('page_name', 'terms_and_conditions_page')->first();
        $privacypolicyPageContent = ContentManagement::where('page_name', 'privacy_policy_page')->first();
        
        return view('admin.content_management.index', compact('webSettingData','homePageContent', 'aboutPageContent', 'vitalBoostPageContent', 'collaboratorPageContent', 'testimonialPageContent', 'faqPageContent', 'callToActionPageContent', 'termsandconditionsPageContent', 'privacypolicyPageContent'));
    }
    ...
    public function updateSiteSettings(Request $request){
        if($request->form_name == 'general_settings'){
            ...
            $pageContent = !empty($request->page_content)?$request->page_content:'';
            // Store content as-is without formatting to preserve HTML structure
            $data = [
    ...
    /**
     * Format HTML content for proper storage with line breaks and indentation.
     *
     * @param string $html
     * @return string
     */
    private function formatHtmlForStorage($html)
    {
        // Add line breaks after block elements and span
        $html = preg_replace('/<\/(div|p|h[1-6]|section|article|header|footer|nav|aside|main|form|fieldset|legend|ul|ol|li|dl|dt|dd|table|thead|tbody|tfoot|tr|td|th|blockquote|pre|address|figure|figcaption|hr|span)>/', "</$1>\n", $html);
        
        // Add line breaks before opening block elements and span
        $html = preg_replace('/<(div|p|h[1-6]|section|article|header|footer|nav|aside|main|form|fieldset|legend|ul|ol|li|dl|dt|dd|table|thead|tbody|tfoot|tr|td|th|blockquote|pre|address|figure|figcaption|hr|span)/', "\n<$1", $html);

...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        //Members List
        $members = User::where('role', 'user')->paginate(10);
    ...
    public function show(string $id)
    {
        ...
        return view('admin.users.show', compact('userDetail', 'paymentHistory'));
    }
    ...
    public function destroy(int $id=0)
    {
        try{
            ...
            return redirect()->back()->with('error', 'Unable to delete user because user is associated with other records. Please try again...!!!');
        }
    }
            ...
            if($status_value == 'active') {
                if($userDetail->role == 'collaborator') {
                    if(!empty($userDetail->email)) {
                        Mail::to($userDetail->email)->send(
                            new CollaboratorActiveMail($userDetail)
                        );
                    }
                } else {
                    if(!empty($userDetail->email)) {
                ...
                if($userDetail->role == 'collaborator') {
                    if(!empty($userDetail->email)) {
                        Mail::to($userDetail->email)->send(
                            new CollaboratorInactiveMail($userDetail)
                        );
                    }
                } else {
                    if(!empty($userDetail->email)) {

...
<body  x-data="{  sidebarOpen: true,  mobileSidebar: false  }"  class="bg-slate-50 antialiased">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <main class="flex-1 p-8 bg-gradient-subtle">
                <div class="space-y-6">
                    ...
                    <div id="faq_categories" class="rounded-lg border bg-card shadow-sm">
                        <div class="p-6">
                            <div class="flex justify-between items-center">
                                <div>
                                    <h3 class="text-2xl font-semibold">FAQ Categories</h3>
                                </div>
                                <div class="w-[220px]">
                                    <input id="search_faq_category" type="text" name="search_faq_category" placeholder="Search FAQ categories..." onkeyup="searchFaqCategories()" class="flex h-10 w-full rounded-md
                                    border border-input
                                    bg-background
                                    px-3 py-2
                                    text-[14px]
                                ...
                                <div class="px-6 py-4 border-t border-gray-100">
                                    <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                                        <div class="text-sm text-gray-500">
                                            Showing <span class="font-semibold text-gray-700">{{ $faqCategories->firstItem() }}</span> 
                                            to <span class="font-semibold text-gray-700">{{ $faqCategories->lastItem() }}</span> 
                                            of <span class="font-semibold text-gray-700">{{ $faqCategories->total() }}</span> faq categories
                                        </div>
                                        <div class="custom-pagination">
                                            {{ $faqCategories->links('pagination::tailwind') }}
                                        </div>
                                    </div>
                                </div>
                            @endif
    ...
    function searchFaqCategories() {
        const searchInput = document.getElementById('search_faq_category');
        const filter = searchInput.value.toLowerCase();
        const table = document.querySelector('#faq_categories table');

...
<head>
    ...
    <title>@yield('title', 'Help Category Detail | Institute for Living Longer - Your Journey to Wellness')</title>
    ...
    <div class="flex-1 flex flex-col">
        ...
        <main class="flex-1 p-8 bg-white ">
            <div class="space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        <x-button-use href="{{route('admin.help-categories')}}" variant="outline" icon="arrow-left" class=" bg-white h-10 w-10 pl-1 pr-0"/>
                    <div>
                        <h1 class="text-3xl font-bold text-left mb-0">Help Category Detail</h1>
                        <p class="text-muted-foreground text-lg">View and manage help category information</p>
                    </div>
                </div>
                <div class="flex gap-2">
                    <!-- <x-button-use label="Delete" variant="outline" icon="trash2" />
                    <x-button-use label="Save Changes" variant="primary" icon="save" /> -->
                    <button type="button" onclick="deleteFromList('{{$helpCategoryDetail->id}}','help_category')" class="rounded-md flex items-center justify-center font-semibold gap-2 transition-all duration-150 select-none px-5 py-2 text-base h-10 border-2 border-primary text-primary hover:bg-primary hover:text-white font-semibold text-[14px]">
                    ...
                    <form id="help_category_delete_form_{{$helpCategoryDetail->id}}" action="{{url('admin/help-categories/'.$helpCategoryDetail->id)}}" method="POST">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
            ...
            <div class="py-4 rounded-lg border bg-card text-card-foreground shadow-sm">
                ...
                <!-- FORM -->
                <form method="POST" id="editHelpCategoryForm" name="editHelpCategoryForm" action="{{ route('admin.help-categories.update', $helpCategoryDetail->id) }}" class="space-y-3 scroll-smooth px-5">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="help_category_id" value="{{ $helpCategoryDetail->id }}">
                    <div class="grid grid-cols-2 gap-4 mt-3">
                        <div class="space-y-2">
                            <x-form.input label="Icon" type="text" name="icon" id="icon" value="{{ $helpCategoryDetail->icon }}" placeholder="Enter Icon*" autocomplete="off" placeholder="Enter Icon*" required />
                        </div>
                        <div class="space-y-2">
                            <x-form.input label="Title" type="text" name="title" id="title" value="{{ $helpCategoryDetail->title }}" placeholder="Enter Title*" autocomplete="off" placeholder="Enter Title*" required />
                        </div>
                        <div class="space-y-2">
                            <x-form.input label="Description" type="text" name="description" id="description" value="{{ $helpCategoryDetail->description }}" placeholder="Enter Description*" autocomplete="off" placeholder="Enter Description*" required />
                        </div>
                        <!-- <div class="space-y-2 z-0">
                            ...
                            :selected="[$helpCategoryDetail->status]"
                            />
                        </div> -->
                    </div>
                </form>
...
<script>
    function updateHelpCategory() {
        document.getElementById('editHelpCategoryForm').submit();
    }
</script>

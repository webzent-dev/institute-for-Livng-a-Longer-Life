...
<head>
    ...
    <title>@yield('title', 'Intro Video Detail | Institute for Living Longer - Your Journey to Wellness')</title>
        ...
        <main class="flex-1 p-8 bg-white ">
            <div class="space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        <x-button-use href="{{route('admin.intro-videos')}}" variant="outline" icon="arrow-left" class=" bg-white h-10 w-10 pl-1 pr-0"/>
                    <div>
                        <h1 class="text-3xl font-bold text-left mb-0">Intro Video Detail</h1>
                        <p class="text-muted-foreground text-lg">View and manage intro video information</p>
                    </div>
                </div>
                <div class="flex gap-2">
                    <!-- <x-button-use label="Delete" variant="outline" icon="trash2" />
                    <x-button-use label="Save Changes" variant="primary" icon="save" /> -->
                    <button type="button" onclick="deleteFromList('{{$introVideoDetail->id}}','intro_video')" class="rounded-md flex items-center justify-center font-semibold gap-2 transition-all duration-150 select-none px-5 py-2 text-base h-10 border-2 border-primary text-primary hover:bg-primary hover:text-white font-semibold text-[14px]">
                    ...
                    <form id="intro_video_delete_form_{{$introVideoDetail->id}}" action="{{url('admin/intro-videos/'.$introVideoDetail->id)}}" method="POST">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
            ...
            <div class="py-4 rounded-lg border bg-card text-card-foreground shadow-sm">
                <div class="flex flex-col space-y-1.5 p-6">
                    <h3 class="text-2xl font-semibold leading-none tracking-tight">Intro Video Information</h3>
                </div>

                <!-- FORM -->
                <form method="POST" id="editIntroVideoForm" name="editIntroVideoForm" action="{{ route('admin.intro-videos.update', $introVideoDetail->id) }}" class="space-y-3 scroll-smooth px-5">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="intro_video_id" value="{{ $introVideoDetail->id }}">
                    <div class="grid grid-cols-2 gap-4 mt-3">
                        <div class="space-y-2">
                            <x-form.input label="Title" type="text" name="title" id="title" value="{{ $introVideoDetail->title }}" placeholder="Enter Title*" autocomplete="off" placeholder="Enter Title*" required />
                        </div>
                        <div class="space-y-2">
                            <x-form.input label="Video URL" type="text" name="video_url" id="video_url" value="{{ $introVideoDetail->video_url }}" placeholder="Enter Video URL*" autocomplete="off" required />
                        </div>
                        <div class="space-y-2 col-span-2">
                            <x-form.textarea label="Description" name="description" id="description" placeholder="Enter Description" autocomplete="off" value="{{ $introVideoDetail->description }}" />
                        </div>
                        <div class="space-y-2 z-0">
                            ...
                            :selected="[$introVideoDetail->status]"
                            />
                        </div>
                    </div>
                </form>
...
<script>
    function updateIntroVideo() {
        document.getElementById('editIntroVideoForm').submit();
    }
</script>

                ...
                <div dir="ltr" data-orientation="horizontal">
                    <div role="tablist" aria-orientation="horizontal" class="grid h-10 w-full grid-cols-3 items-center justify-center rounded-md bg-muted p-1 text-muted-foreground" tabindex="0">
                        <button role="tab" aria-selected="true" data-state="active" data-tab="institute_orders" aria-controls="members" type="button" class="inline-flex items-center justify-center rounded-sm px-3 py-1.5 text-sm font-medium transition-all data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm">
                            Institute Orders ({{ $instituteOrders->total() }})
                        </button>
                        <button role="tab" aria-selected="false" data-state="inactive" data-tab="collaborator_orders" aria-controls="collaborators" type="button" class="inline-flex items-center justify-center rounded-sm px-3 py-1.5 text-sm font-medium transition-all">
                            Collaborator Orders ({{ $collaboratorOrders->total() }})
                        </button>
                        ...
                        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
                            <div class="flex flex-col space-y-1.5 p-6">
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="w-[220px]">
                                        <input id="search" type="text" name="search" placeholder="Search orders..." onkeyup="searchOrders()" class="flex h-10 w-full rounded-md
                                        border border-input
                                        bg-background
                                        px-3 py-2
                                        text-[14px]
                            ...
                            <div class="p-6 pt-0">
                                ...
                                <!-- PAGINATION UI -->
                                @if($instituteOrders->hasPages())
                                    <div class="px-6 py-4 border-t border-gray-100">
                                        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                                            <div class="text-sm text-gray-500">
                                                Showing <span class="font-semibold text-gray-700">{{ $instituteOrders->firstItem() }}</span> 
                                                to <span class="font-semibold text-gray-700">{{ $instituteOrders->lastItem() }}</span> 
                                                of <span class="font-semibold text-gray-700">{{ $instituteOrders->total() }}</span> orders
                                            </div>
                                            <div class="custom-pagination">
                                                {{ $instituteOrders->links('pagination::tailwind') }}
                                            </div>
                                        </div>
                                    </div>
                                @endif
                            ...
                            <div class="p-6 pt-0">
                                <div class="relative w-full overflow-auto">
                                    <table class="w-full caption-bottom text-sm">
                                        <thead class="[&_tr]:border-b">
                                            <tr class="border-b transition-colors data-[state=selected]:bg-muted hover:bg-muted/50">
                                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Collaborator Name</th>
                                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Order Number</th>
                                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Customer</th>
                                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Total</th>
                                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Status</th>
                                        ...
                                        <tbody class="[&_tr:last-child]:border-0">
                                            @forelse($collaboratorOrders as $order)
                                                @php 
                                                    ...
                                                    $collaborator = DB::table('users')->where('id', $product->user_id)->first();
                                                    $order->collaborator_name = $collaborator->first_name . ' ' . $collaborator->last_name;
                                                    $order->collaborator_email = $collaborator->email;
                                                @endphp
                                            <tr class="border-b transition-colors data-[state=selected]:bg-muted hover:bg-muted/50">
                                                <td class="p-4 align-middle">
                                                    <p class="font-medium">{{$order->collaborator_name}}</p>
                                                    <p class="text-sm text-muted-foreground">{{$order->collaborator_email}}</p>
                                                </td>
                                                <td class="p-4 align-middle">{{$order->order_number}}</td>
                                                <td class="p-4 align-middle">
                                                    <div>
                                                ...
                                                <td class="p-4 align-middle">{{$order->created_at}}</td>
                                                <td class="p-4 justify-items-center">
                                                    <x-button-use href="{{ route('admin.order.details', $order->o_id) }}" label="View" variant="outline" icon="eye" class="pl-0 pr-0 w-24 h-10"/>
                                                </td>
                                            </tr>
                                            @empty
                                            <tr class="border-b transition-colors data-[state=selected]:bg-muted hover:bg-muted/50">
                                ...
                                <!-- PAGINATION UI -->
                                @if($collaboratorOrders->hasPages())
                                    <div class="px-6 py-4 border-t border-gray-100">
                                        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                                            <div class="text-sm text-gray-500">
                                                Showing <span class="font-semibold text-gray-700">{{ $collaboratorOrders->firstItem() }}</span> 
                                                to <span class="font-semibold text-gray-700">{{ $collaboratorOrders->lastItem() }}</span> 
                                                of <span class="font-semibold text-gray-700">{{ $collaboratorOrders->total() }}</span> orders
                                            </div>
                                            <div class="custom-pagination">
                                                {{ $collaboratorOrders->links('pagination::tailwind') }}
                                            </div>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
                            <div class="flex flex-col space-y-1.5 p-6">
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="w-[220px]">
                                        <input id="search_collaborator" type="text" name="search_collaborator" placeholder="Search collaborators..." onkeyup="searchCollaborators()" class="flex h-10 w-full rounded-md
                                        border border-input
                                        bg-background
                                        px-3 py-2
                                        text-[14px]
                                    ...
                                    <table class="w-full caption-bottom text-sm">
                                        ...
                                        <tbody class="[&_tr:last-child]:border-0">
                                            @forelse($collaborators as $collaborator)
                                                ...
                                                $collaboratorProducts = DB::table('products')->where('user_id', $collaborator->id)->get();
                                                ...
                                                $collaborator->totalOrders = DB::table('order_items')->whereIn('product_id', $collaboratorProductIds)->count();
                                                $collaborator->totalRevenue = DB::table('order_items')->whereIn('product_id', $collaboratorProductIds)->sum('total');
                                                $collaborator->totalProducts = DB::table('order_items')->whereIn('product_id', $collaboratorProductIds)->sum('quantity');
...
function searchOrders() {
    ...
    const table = document.querySelector('#institute_orders table');
...
function searchCollaborators() {
    const searchInput = document.getElementById('search_collaborator');
    const filter = searchInput.value.toLowerCase();
    const table = document.querySelector('#collaborator_analytics table');
    const rows = table.getElementsByTagName('tr');

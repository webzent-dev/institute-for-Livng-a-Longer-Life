    ...
    <body x-data="{  sidebarOpen: true,  mobileSidebar: false  }"  class="bg-slate-50 antialiased">
            ...
            <main class="flex-1 p-4 md:p-6 overflow-y-auto scrollbar-custom bg-white">
               <div class="p-6  min-h-screen">
                    ...
                    <x-ui.modal name="add-video-testimonial-modal" title="Add New Video Testimonial" size="md" class="max-w-2xl overflow-y-auto scrollbar-custom ">
                        ...
                        <form method="POST" action="{{ route('admin.video-testimonials.store') }}" class="space-y-3 overflow-y-auto scrollbar-custom max-h-[60vh] scroll-smooth px-5" enctype="multipart/form-data">
                            @csrf
                    ...
                    <div id="video_testimonials" class="rounded-lg border bg-card text-card-foreground shadow-sm">
                        <div class="flex flex-col space-y-1.5 p-6">
                            <div class="flex justify-between items-center">
                                ...
                                <div class="w-[220px]">
                                    <input id="search_video_testimonial" type="text" name="search_video_testimonial" placeholder="Search testimonials..." onkeyup="searchVideoTestimonials()" class="flex h-10 w-full rounded-md
                                    border border-input
                                    bg-background
                                    px-3 py-2
                                    text-[14px]
                                        ...
                                        <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                            ...
                                            <td class="p-4 align-middle">
                                                <div class="flex gap-2">
                                                    ...
                                                    <form id="video_testimonial_delete_form_{{$testimonial->id}}" action="{{url('admin/video-testimonials/'.$testimonial->id)}}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                </div>
        ...
        $(document).ready(function () {
            ...
            $('#thumbnail').change(function() {
                ...
                if (fileName) {
                    $('#thumbnail-count').text('Selected: ' + fileName).removeClass('hidden');
                } else {
                    $('#thumbnail-count').addClass('hidden');
                }
            });
        ...
        function searchVideoTestimonials() {
            const searchInput = document.getElementById('search_video_testimonial');
            const filter = searchInput.value.toLowerCase();
            const table = document.querySelector('#video_testimonials table');

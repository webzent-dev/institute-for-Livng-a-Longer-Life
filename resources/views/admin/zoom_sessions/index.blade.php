                        ...
                        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
                            <div class="flex flex-col space-y-1.5 p-6">
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="w-[220px]">
                                        <input id="search_zoom_session" type="text" name="search_zoom_session" placeholder="Search sessions..." onkeyup="searchZoomSession()" class="flex h-10 w-full rounded-md
                                        border border-input
                                        bg-background
                                        px-3 py-2
                                        text-[14px]
                            ...
                            <div class="p-6 pt-0">
                                ...
                                <!-- PAGINATION UI -->
                                @if($zoom_sessions->hasPages())
                                    <div class="px-6 py-4 border-t border-gray-100">
                                        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                                            <div class="text-sm text-gray-500">
                                                Showing <span class="font-semibold text-gray-700">{{ $zoom_sessions->firstItem() }}</span> 
                                                to <span class="font-semibold text-gray-700">{{ $zoom_sessions->lastItem() }}</span> 
                                                of <span class="font-semibold text-gray-700">{{ $zoom_sessions->total() }}</span> sessions
                                            </div>
                                            <div class="custom-pagination">
                                                {{ $zoom_sessions->links('pagination::tailwind') }}
                                            </div>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <div class="rounded-lg border bg-card text-card-foreground shadow-sm">
                            <div class="flex flex-col space-y-1.5 p-6">
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="w-[220px]">
                                        <input id="search_recording" type="text" name="search_recording" placeholder="Search recordings..." onkeyup="searchRecording()" class="flex h-10 w-full rounded-md
                                        border border-input
                                        bg-background
                                        px-3 py-2
                                        text-[14px]
                            ...
                            <div class="p-6 pt-0">
                                ...
                                @if($recordings->hasPages())
                                    <div class="px-6 py-4 border-t border-gray-100">
                                        <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                                            <div class="text-sm text-gray-500">
                                                Showing <span class="font-semibold text-gray-700">{{ $recordings->firstItem() }}</span> 
                                                to <span class="font-semibold text-gray-700">{{ $recordings->lastItem() }}</span> 
                                                of <span class="font-semibold text-gray-700">{{ $recordings->total() }}</span> recordings
                                            </div>
                                            <div class="custom-pagination">
                                                {{ $recordings->links('pagination::tailwind') }}
                                            </div>
                                        </div>
                                    </div>
                                @endif
...
function searchRecording() {
    const searchInput = document.getElementById('search_recording');
    const filter = searchInput.value.toLowerCase();

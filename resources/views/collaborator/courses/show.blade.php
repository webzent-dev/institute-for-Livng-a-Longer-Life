<!DOCTYPE html>
    ...
    <body  x-data="{  sidebarOpen: true,  mobileSidebar: false  }"  class="bg-slate-50 antialiased">
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <x-button-use href="{{ route('collaborator.courses') }}"   variant="outline" icon="arrow-left" class=" bg-white h-10 w-10 pl-1 pr-0"/>
                            <div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <form id="course_delete_form_{{$courseDetail->id}}" action="{{ route('collaborator.courses.delete', $courseDetail->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
                    ...
                    <div class="py-4 rounded-lg border bg-card text-card-foreground shadow-sm">
                        ...
                        <form method="POST" id="editCourseForm" name="editCourseForm" action="{{ route('collaborator.courses.update', $courseDetail->id) }}" enctype="multipart/form-data" class="space-y-3 overflow-y-auto scrollbar-custom  scroll-smooth px-5">
                            @csrf
                            @method('PUT')
        ...
        function removeImage(imageId) {
            ...
            if (r == true) {
                $.ajax({
                    url: "{{ route('collaborator.courses.removeImage') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        image_id: imageId

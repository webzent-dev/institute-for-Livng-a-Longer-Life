...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body x-data="{  sidebarOpen: true,  mobileSidebar: false  }"  class="bg-slate-50 antialiased">
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <x-button-use href="{{ route('collaborator.products') }}"  variant="outline" icon="arrow-left" class=" bg-white h-10 w-10 pl-1 pr-0"/>
                            <div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <form id="product_delete_form_{{$productDetail->id}}" action="{{ route('collaborator.products.delete',$productDetail->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
                    ...
                    <div class="py-4 rounded-lg border bg-card text-card-foreground shadow-sm">
                        ...
                        <form method="POST" id="editProductForm" name="editProductForm" action="{{ route('collaborator.products.update', $productDetail->id) }}" enctype="multipart/form-data" class="space-y-3 overflow-y-auto scrollbar-custom scroll-smooth px-5">
                            @csrf
                            @method('PUT')
                        ...
                        <div class="p-6 pt-0">
                            <div class="grid grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-black">${{number_format(0, 2)}}</p>
                                </div>
                                <div>
                                    <label class="text-md font-medium text-muted-foreground">Revenue</label>
                                    <p class="text-lg text-black">${{number_format(0, 2)}}</p>
                                </div>
                                <div>
        ...
        function removeImage(imageId) {
            ...
            if (r == true) {
                $.ajax({
                    url: "{{ route('collaborator.products.removeImage') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        image_id: imageId

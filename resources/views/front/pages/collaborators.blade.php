    ...
    <main class="flex-1">
        ...
        <section class="gradient-subtle py-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                ...
                <p class="text-xl text-muted-foreground max-w-3xl mx-auto">
                Learn from a network of world-class physicians and health practitioners,
                each bringing specialized expertise to your wellness journey. Access
                their exclusive courses and recommended products.
                </p>
            </div>
        </section>
        ...
        <section class="pt-20 bg-background">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                {{-- Filter Controls --}}
                <div class="mb-10 bg-white p-6 rounded-2xl shadow-soft border border-gray-200">
                    <div class="flex flex-col md:flex-row items-center justify-center gap-4 mb-6">
                        <div>
                            <h2 class="text-3xl font-bold ">Our Collaborators </h2>
                            <p class="text-muted-foreground text-md mt-1">Find experts by name, specialty, or status</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                @php
                                    $specialties = $collaborators
                                        ->pluck('Specialty')
                                        ->filter()   // null / empty hata dega
                                        ->unique()
                                        ->sort()
                                        ->values();
                                @endphp
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div id="collaboratorsGrid" class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($collaborators as $c)
                        <div class="flex flex-col card border-2 rounded-2xl {{ $c['featured'] ? 'border-primary shadow-medium' : 'shadow-soft' }} hover:border-primary py-4  collaborator-card"
                            data-name="{{ strtolower($c['first_name']) }}"
                            data-specialty="{{ $c['Specialty'] }}"
                            data-featured="{{ $c['featured'] ? 'featured' : 'not-featured' }}">
                            <div class="card-header pt-1 p-6 pb-0 ">
                                <div class="flex justify-between items-start mb-4">
                                    <x-ui.avatar name="{{ $c['first_name'] }}" size="4" />
                                    <x-badge class=""> Featured </x-badge>
                                    @if ($c['featured'])
                                        <x-badge class=""> Featured </x-badge>
                                    @endif
                                </div>
                                   <h2 class="font-semibold tracking-tight text-2xl   text-left">
                                    {{ $c['first_name'] }} {{ $c['last_name'] }}
                                </h2>
                                {{-- <div class="space-y-1 "> --}}
                                    <p class="text-primary font-semibold text-[16px]">{{ $c['Specialty'] }}</p>
                                    <p class="font-semibold text-muted-foreground text-[14px]  ">{{ $c['professional_credentials'] }}</p>
                                {{-- </div> --}}
                            </div>
                            {{-- <div class="space-y-1 px-6 ">
                                <p class="text-muted-foreground mb-2 flex-1">
                                    {{ $c['collaborator_massge'] }}
                                </p>
                            </div> --}}
                            ...
                            <div class="p-6 pt-0 flex-1 flex flex-col">
                                ...
                                <p class="text-muted-foreground mb-6 flex-1 text-[16px]">{{ $c['collaborator_massge'] }}</p>
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 justify-center pt-2">
                                        <x-button-use href="{{ route('membership') }}" label="View Profile" variant="primary"  icon="award" size="md"/>
                                        <x-button-use href="{{ route('intro-videos') }}" label="Visit Store" variant="outline" icon="store" size="md"/>
                                </div>

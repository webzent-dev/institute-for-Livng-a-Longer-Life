...
{{-- <div class="flex justify-center px-4 sm:px-6 lg:px-8 py-4">
    <nav class="bg-card border border-border rounded-2xl shadow-lg px-2 py-2 flex flex-wrap gap-1">  
        @php
            $menuItems = [
                ['title' => 'Dashboard', 'url' => route('member.dashboard'), 'icon' => 'layout-dashboard'],
                ['title' => 'Videos', 'url' => route('member.video-library'), 'icon' => 'video'],
                ['title' => 'Store', 'url' => route('member.store'), 'icon' => 'store'],
                ['title' => 'Profile', 'url' => route('member.profile'), 'icon' => 'user'],
                ['title' => 'Security', 'url' => route('member.security'), 'icon' => 'shield'],
                ['title' => 'Subscription', 'url' => route('member.subscription'), 'icon' => 'credit-card'],
                ['title' => 'Orders', 'url' => route('member.orders'), 'icon' => 'package'],
                ['title' => 'Plans', 'url' => route('member.plans'), 'icon' => 'arrow-up-down'],
                ['title' => 'Payments', 'url' => route('member.payments'), 'icon' => 'receipt'],
            ];
        @endphp
        ...
        @foreach($menuItems as $item)
            <a
                href="{{ $item['url'] }}"
                class="{{ request()->url() === $item['url'] ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground hover:bg-muted' }} flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200"
            >

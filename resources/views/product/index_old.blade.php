                ...
                <div class="p-6 bg-gray-50 min-h-screen">
                    ...
                    <div class="flex items-center justify-between mb-5">
                        ...
                        @if(auth()->user()->role === 'collaborator')
                            <a href="{{ route('products.create') }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium shadow">
                                <i data-lucide="plus" class="w-4 h-4"></i>
                                Add Product
                            </a>
                        @else
                            <a href="{{ route('admin.products.create') }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium shadow">
                                <i data-lucide="plus" class="w-4 h-4"></i>
                                Add Product
                            </a>
                        @endif
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3">
                                            @if($product->product_type === 'supplement')
                                                <span class="px-2 py-1 text-xs rounded bg-green-100 text-green-700">Supplement</span>
                                            @elseif($product->product_type === 'vital_boost')
                                                <span class="px-2 py-1 text-xs rounded bg-blue-100 text-blue-700">Vital Boost</span>
                                            @elseif($product->product_type === 'guide')
                                                <span class="px-2 py-1 text-xs rounded bg-yellow-100 text-yellow-700">Guide</span>
                                            @else
                                                <span class="px-2 py-1 text-xs rounded bg-gray-100 text-gray-700">Book</span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-3">${{ number_format($product->price, 2) }}</td>
                                        <td class="px-4 py-3">{{ $product->stock_quantity }}</td>
                                        <td class="px-4 py-3">
                                            <span class="px-2 py-1 text-xs rounded status-badge
                                        ...
                                        <td class="px-4 py-3 text-right flex justify-end space-x-2">
                                            ...
                                            @else
                                                <a href="{{ route('admin.products.edit', $product->id) }}" class="text-blue-600 hover:text-blue-800" title="Edit">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                                        <path d="M17.414 2.586a2 2 0 010 2.828l-10 10a2 2 0 01-.708.414l-5 1a1 1 0 01-1.212-1.212l1-5a2 2 0 01.414-.708l10-10a2 2 0 012.828 0zM15 5l-1-1L4 14v1h1L15 5z"/>
                                                    </svg>
                                                </a>
